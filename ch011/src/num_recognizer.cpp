...
static char const *INPUT_NAMES[] = {"Input3"};             // 模型输入参数名
static char const *OUTPUT_NAMES[] = {"Plus214_Output_0"};  // 模型输出参数名
static constexpr int64_t INPUT_WIDTH = 28;                 // 模型输入图片宽度
static constexpr int64_t INPUT_HEIGHT = 28;                // 模型输入图片高度
static std::array<int64_t, 4> const input_shape{
    1, 1, INPUT_WIDTH, INPUT_HEIGHT};  // 输入数据的形状（各维度大小）
static std::array<int64_t, 2> const output_shape{1, 10};  // 输出数据的形状（各维度大小）
...
static float byte2float(png_byte b)
{
    return b < 128 ? 1 : 0;
}
...
static float get_float_color_in_png(unsigned int x,
                                    unsigned int y,
...
extern "C"
{
    ...
    {
        env = Ort::Env{static_cast<OrtThreadingOptions const *>(nullptr)};
        memory_info = Ort::MemoryInfo::CreateCpu(OrtDeviceAllocator, OrtMemTypeCPU);
    }

    void num_recognizer_create(char const *model_path, Recognizer **out_recognizer)
    {
    ...
    int num_recognizer_recognize(Recognizer *recognizer, float *input_image, int *result)
    {
        ...
        auto output_tensor = Ort::Value::CreateTensor<float>(
            memory_info, results.data(), results.size(), output_shape.data(), output_shape.size());

        recognizer->session.Run(Ort::RunOptions{nullptr},
                                INPUT_NAMES,
                                &input_tensor,
                                1,
                                OUTPUT_NAMES,
    ...
    int num_recognizer_recognize_png(Recognizer *recognizer, char const *png_path, int *result)
    {
        int ret = 0;
        ...
        for (unsigned int y = 0; y < INPUT_HEIGHT; ++y)
        {
            for (unsigned int x = 0; x < INPUT_WIDTH; ++x)
            {
                ...
                for (unsigned int png_y = y * png_height / INPUT_HEIGHT;
                     png_y < (y + 1) * png_height / INPUT_HEIGHT;
                     ++png_y)
                {
                    for (unsigned int png_x = x * png_width / INPUT_WIDTH;
                         png_x < (x + 1) * png_width / INPUT_WIDTH;
                         ++png_x)
                    {
                        res += get_float_color_in_png(
                            png_x, png_y, png_width, png_height, color_type, png_data);
                        ++n;
                    }
                }
